...
export default function Dashboard() {
  ...
  const freeTriesLeft = Math.max(0, FREE_TRIES_LIMIT - totalGenerations);
  const paidGenerations = Math.max(0, totalGenerations - FREE_TRIES_LIMIT);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl shadow-sm border border-brand-200 p-6">
          ...
          <div className="w-full bg-brand-100 rounded-full h-3">
            <div 
              ...
              style={{ width: `${(freeTriesLeft / FREE_TRIES_LIMIT) * 100}%` }}
            />
          </div>
          <p className="text-xs text-brand-400 mt-2">{freeTriesLeft} of {FREE_TRIES_LIMIT} free generations remaining</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-brand-200 p-6">
          ...
          <p className="text-xs text-brand-400">Rs. {COST_PER_GENERATION} per generation after {FREE_TRIES_LIMIT} free tries</p>
        </div>
      </div>
